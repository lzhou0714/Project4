...
ValueType* RadixTree<ValueType>:: search(std::string key) const
{
    ...
    if (currNode == nullptr)
        ...
        while (currKey.size()>0)
        {
            ...
            if (currNode ==nullptr || currKey.size() < currNode->k.size()) //end of word/end of tree is reached, word not in tree
                break;

            else if (currKey == currNode->k && currNode->endOfString ==true) //word ends at current node
                return &(currNode->v);

            else //part of word matches to current node, split word accordingly and proceed to next node
            {
